/**
 * ISO 3166-1 alpha-2 codes with English names. Generated rather than typed by
 * hand, and stored as codes so the display name can be localised later without
 * a data migration.
 *
 * Flags are served from `public/flags/<code>.svg`; run `scripts/fetch-flags.mjs`
 * to populate that directory. `Flag` degrades to a code chip when a file is
 * missing, so nothing breaks before the assets land.
 */
export type Country = {
  /** Lowercase in URLs, uppercase here to match the ISO standard. */
  code: string;
  name: string;
};
...
export function flagSource(code: string): string {
